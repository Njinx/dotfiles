...
func main() {
	...
	localtime := clock.Local().
		Output(time.Second, func(now time.Time) bar.Output {
			return outputs.Text(now.Format("03:04 PM"))
		})
	...
	battSummary, battDetail := split.New(battery.All().Output(func(i battery.Info) bar.Output {
		...
		out.Append(outputs.Pango(
			pango.Icon("mdi-"+iconName).Alpha(0.75),
			spacer,
	...
	wifiName, wifiDetails := split.New(wlan.Any().Output(func(i wlan.Info) bar.Output {
		...
		out.Append(outputs.Pango(
			pango.Icon("mdi-wifi").Alpha(0.75),
			spacer,
			pango.Text(truncate(i.SSID, -32)),
		).OnClick(click.Left(func() {
		...
		out.Append(outputs.Pango(
			pango.Icon("mdi-access-point").Alpha(0.75),
			spacer,
	...
	vol := volume.DefaultMixer().Output(func(v volume.Volume) bar.Output {
		if v.Mute {
			return outputs.
				Pango(pango.Icon("fa-volume-mute").Alpha(0.75), spacer, "MUT").
				Color(colors.Scheme("degraded"))
		}
		iconName := "off"
		pct := v.Pct()
		...
		return outputs.Pango(
			pango.Icon("fa-volume-"+iconName).Alpha(0.75),
			spacer,
			pango.Textf("%2d%%", pct),
		)
	})
	...
	loadAvg := sysinfo.New().Output(func(s sysinfo.Info) bar.Output {
		out := outputs.Pango(
			pango.Icon("mdi-desktop-tower").Alpha(0.75),
			spacer,
			pango.Textf("%0.2f %0.2f %0.2f", s.Loads[0], s.Loads[1], s.Loads[2]),
		)
	...
	uptime := sysinfo.New().Output(func(s sysinfo.Info) bar.Output {
		...
		if u.Hours() < 24 {
			uptimeOut = pango.Textf("%0d:%02d",
				int(u.Hours()), int(u.Minutes())%60)
		} else {
			uptimeOut = pango.Textf("%dd %02dh",
				int(u.Hours()/24), int(u.Hours())%24)
		}
		return pango.Icon("mdi-trending-up").Alpha(0.75).Concat(spacer, uptimeOut)
	})

	freeMem := meminfo.New().Output(func(m meminfo.Info) bar.Output {
		out := outputs.Pango(
			pango.Icon("material-memory").Alpha(0.75),
			spacer,
	...
	swapMem := meminfo.New().Output(func(m meminfo.Info) bar.Output {
		return outputs.Pango(
			pango.Icon("mdi-swap-horizontal").Alpha(0.75),
			spacer,
			format.IBytesize(m["SwapTotal"]-m["SwapFree"]),
			spacer,
			pango.Textf("(% 2.0f%%)", (1-m.FreeFrac("Swap"))*100.0),
		)
	})
	...
	temp := cputemp.New().
		...
		Output(func(temp unit.Temperature) bar.Output {
			out := outputs.Pango(
				pango.Icon("mdi-fan").Alpha(0.75), spacer,
	...
	netsp := netspeed.New(iface).
		...
		Output(func(s netspeed.Speeds) bar.Output {
			return outputs.Pango(
				pango.Icon("fa-upload").Alpha(0.75), spacer, pango.Textf("%7s", format.Byterate(s.Tx)),
				pango.Text(" ").Small(),
				pango.Icon("fa-download").Alpha(0.75), spacer, pango.Textf("%7s", format.Byterate(s.Rx)),
			)
		})
	...
	formatDiskSpace := func(i diskspace.Info, icon string) bar.Output {
		out := outputs.Pango(
			pango.Icon(icon).Alpha(0.75), spacer, format.IBytesize(i.Available))
	...
	sysMode := mainModal.Mode("sysinfo").
		SetOutput(makeIconOutput("material-memory")).
		Add(loadAvg).
		Detail(uptime).
		Add(freeMem).
		Detail(swapMem, temp)
